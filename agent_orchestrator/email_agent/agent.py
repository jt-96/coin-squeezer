...
# from auth_helper import get_workspace_access_token 
...
email_agent = Agent(
    ...
    Once finished, respond with "Email has been sent!" to finish.
    """,
    tools=[
        McpToolset(
            connection_params=StdioConnectionParams(
                server_params=StdioServerParameters(
                    command="npx",
                    args= ["-y", "@google/workspace-mcp-server", "--credentials", "/secrets/credentials.json", "--stdio"],
                ),
                timeout=20
            )
        )
