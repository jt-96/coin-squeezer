...
from .carrefour_sites_list import carrefour_sites

class ScrapItem(BaseModel):
    product_name: str = Field(description="The name of the product.")
    product_price: str = Field(description="The current price of the product.")
    product_store: Literal["Carrefour"] = Field(default="Carrefour", description="The store from where the product comes from.")
...
async def init_agent_callback(callback_context):
    callback_context.state["allowed_carrefour_sites"] = carrefour_sites
...
carrefour_scraper_agent = Agent(

    model='gemini-3.5-flash',
    name='carrefour_scraper_agent',
    description='A web scraper that extracts names and prices from the Carrefour Supermarket site',
    instruction=""" You are a web scraper and data extrator specialist.
    Your job is to scrap the contents of websites, and obtain the name and price of each of the links provided in {allowed_carrefour_sites} using the tools available.
